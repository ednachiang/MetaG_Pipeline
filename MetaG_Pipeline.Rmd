...
Here, I'll only be talk about [MetaBAT](https://bitbucket.org/berkeleylab/metabat) because that's the only program I've used so far.  
...
## 10B) Bin Contigs  
Please refer to [MetaBAT](https://bitbucket.org/berkeleylab/metabat) for parameters to set for your binning. There are lots of options and it's up to you to select what's best for your purposes. I don't include those parameters in the code displayed below.  
```{r, eval=F}
...
```
* `path/to/rpsblast`
    + Replace with your path to [rpsblast](http://nebc.nox.ac.uk/bioinformatics/docs/rpsblast.html)
...
To make sense of the dbCAN output, I parse it to create a table that lists each ORF with a predicted CAZyme and its top CAZyme hit, as well as a count of each CAZyme class. Here are two scripts for this:
