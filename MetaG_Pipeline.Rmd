...
This guide does not touch upon appropriate experimental design, DNA preparation, or sequencing methods. This pipeline is for paired end data only, not singled end. While many of the steps and concepts are translatable to single end data, many of the software programs are incompatible with single end data.  
...
* `LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36`
  ...
  + The numbers are the default parameters. Refer to Trimmomatic's website for more info.
...
I chose Trimmomatic over these two software simply because it is more efficient to use one program instead of two.  
...
However, if you do not have access to your host's genome, I do not have an answer for removing host contamination. In this case, I suggest that you (1) do some googling (in case an answer has been found since I wrote this pipeline), and (2) move to step 5.  
...
### D) Convert SAM file to BAM file  
...
### E) Extract unmapped reads (non-host)  
```{r, eval=F}
...
### F) Sort BAM file to organize paired reads  
```{r}
samtools sort -n /path/to/unmap.bam -o /path/to/unmap.sorted.bam
```
  
### G) Convert BAM to fastq  
```{r, eval=F}
...
This step is an alternative to binning and gives you a quick look at what taxa comprise your metagenome.  
...
There are new assemblers being developed all the time. I've tested 3 assemblers:  
...
##### File pairs
This is if you have 2 fastq files per metagenome.
```{r, eval=F}
...
# 11) Predict open reading frames  
